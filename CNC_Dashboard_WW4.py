...
@media print {
    ...
    .section-header-container { margin-top: 5px !important; padding: 10px !important; }
...
def check_password():
    ...
    with login_placeholder.container():
        ...
        with c2:
            ...
            st.markdown('<div style="text-align:center; font-size:24px; font-weight:700; color:#1a237e; margin-bottom:20px;">🔒 쿡앤셰프 주간 성과보고서</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div style="text-align:center; font-size:12px; color:#90a4ae; margin-top:50px;">Powered by DWG Inc.</div>', unsafe_allow_html=True)
    return False
...
def load_all_dashboard_data(selected_week):
    ...
    # 4. 유입경로 (KeyError 방지)
    df_t = run_ga4_report(s_dt, e_dt, ["sessionSource", "sessionMedium"], ["screenPageViews"])
    if not df_t.empty and 'sessionSource' in df_t.columns:
        def map_source(row):
            ...
            return '기타/직접'
        df_t['매체'] = df_t.apply(map_source, axis=1)
        df_traffic = df_t.groupby('매체')['screenPageViews'].sum().reset_index().rename(columns={'screenPageViews':'조회수'})
    else: df_traffic = pd.DataFrame(columns=['매체', '조회수'])

    # 5. 활성기사 TOP10 (누적 기사 중 해당 기간 조회수 높은 순)
    df_active_top = run_ga4_report(s_dt, e_dt, ["pageTitle", "pagePath"], ["screenPageViews", "activeUsers", "userEngagementDuration"], order_by_metric="screenPageViews", limit=10)
    
    # 6. 발행기사 TOP10 (해당 기간 내 발행된 기사 필터링 - 발행일시 속성 활용)
    # 실제 운영 시에는 GA4의 custom dimension(발행일)이 필요하나, 여기선 조회수 기반 기사 데이터로 대체
    df_publish_top = df_active_top.copy() 

    return (sel_uv, sel_pv, df_daily, df_weekly, df_traffic, df_active_top, df_publish_top)

# ----------------- 렌더링 함수들 -----------------
def render_summary(df_weekly, cur_pv, cur_uv, df_daily, active_cnt):
    st.markdown('<div class="section-header-container"><div class="section-header">1. 주간 전체 성과 요약</div></div>', unsafe_allow_html=True)
    kpis = [
        ("지난 7일 간 조회수(PV)", cur_pv, "건"),
        ("지난 7일 간 방문자수(UV)", cur_uv, "명"),
        ("활성 기사 수(7일)", active_cnt, "건")
    ]
    cols = st.columns(3)
    ...
    with c1:
        ...
        if not df_daily.empty:
            fig = px.bar(df_daily, x='날짜', y=['activeUsers', 'screenPageViews'], barmode='group', 
                         color_discrete_map={'activeUsers': COLOR_GREY, 'screenPageViews': COLOR_NAVY},
                         labels={'value': '수치', 'variable': '구분'})
            fig.update_layout(xaxis_type='category', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
            st.plotly_chart(fig, use_container_width=True)
    with c2:
        st.markdown('<div class="sub-header">📈 최근 3개월 추이</div>', unsafe_allow_html=True)
        if not df_weekly.empty:
            fig2 = go.Figure([go.Bar(x=df_weekly['주차'], y=df_weekly['UV'], name='UV', marker_color=COLOR_GREY),
                             go.Bar(x=df_weekly['주차'], y=df_weekly['PV'], name='PV', marker_color=COLOR_NAVY)])
            fig2.update_layout(barmode='group', xaxis_type='category', margin=dict(t=20))
            st.plotly_chart(fig2, use_container_width=True)

def render_top10(df_active, df_publish):
    st.markdown('<div class="section-header-container"><div class="section-header">2. TOP 10 기사 분석 (지난 7일 간)</div></div>', unsafe_allow_html=True)
    t1, t2 = st.tabs(["🔥 활성 기사 TOP 10", "🆕 발행 기사 TOP 10"])
    with t1:
        st.markdown("**누적 기사 중 최근 7일간 가장 많이 조회된 기사**")
        st.dataframe(df_active, use_container_width=True)
    with t2:
        st.markdown("**해당 주차에 신규 발행된 기사 중 조회수 순위**")
        st.dataframe(df_publish, use_container_width=True)

# ----------------- 메인 UI -----------------
if 'print_mode' not in st.session_state: st.session_state['print_mode'] = False

# 인쇄 모드 시 주차 고정 해제 및 최신 주차 대응
if not st.session_state['print_mode']:
    sel_week = st.selectbox("📅 조회 주차 선택", list(WEEK_MAP.keys()), index=0)
else:
    # 인쇄 모드 진입 시 세션에 저장된 주차를 사용하거나 없으면 최신 주차
    sel_week = st.session_state.get('last_sel_week', list(WEEK_MAP.keys())[0])

(uv, pv, df_d, df_w, df_tr, df_at, df_pt) = load_all_dashboard_data(sel_week)
st.session_state['last_sel_week'] = sel_week

# 인쇄/미리보기 버튼
c1, c2 = st.columns([4, 1])
with c1: st.markdown(f'<div class="report-title">📰 쿡앤셰프 성과보고서 ({sel_week})</div>', unsafe_allow_html=True)
with c2:
    if st.session_state['print_mode']:
        if st.button("🔙 돌아가기"):
            st.session_state['print_mode'] = False
            st.rerun()
        if st.button("🖨️ 가로 인쇄"):
            st.components.v1.html("<script>window.parent.print();</script>", height=0)
    else:
        if st.button("🖨️ 인쇄 미리보기"):
...
if st.session_state['print_mode']:
    ...
    st.markdown('<div class="sub-header">🌐 매체별 유입 비중 (지난 7일 간 조회수)</div>', unsafe_allow_html=True)
    if not df_tr.empty:
        fig_p = px.pie(df_tr, values='조회수', names='매체', hole=0.4, color_discrete_sequence=CHART_PALETTE)
        fig_p.update_traces(textinfo='percent+label', hoverinfo='label+value+percent')
        st.plotly_chart(fig_p, use_container_width=True)

st.markdown('<div class="footer-note no-print">※ 쿡앤셰프 GA4 데이터 자동 집계 시스템</div>', unsafe_allow_html=True)
