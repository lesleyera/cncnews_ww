...
def load_all_dashboard_data(selected_week):
    ...
    if not df_weekly.empty:
        df_weekly = df_weekly.sort_values('sort_key')
...
def render_summary(df_weekly, cur_pv, cur_uv, new_ratio, search_ratio, df_daily, active_article_count):
    ...
    with c1:
        ...
        if not df_daily.empty:
            ...
            fig.update_layout(xaxis_type='category') # X축 범례 타입 강제 고정
    ...
    with c2:
        ...
        if not df_weekly.empty:
            ...
            fig2.update_layout(barmode='group', plot_bgcolor='white', margin=dict(t=0), xaxis_type='category')
...
# ----------------- 뷰 렌더링 -----------------
if st.session_state['print_mode']:
    st.info("💡 인쇄 미리보기: 각 페이지별로 나누어 출력됩니다.")
